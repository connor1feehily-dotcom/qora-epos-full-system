...
**May 2, 2026 — Real AI delivery-docket scanning (was previously fake):**
- ✅ Replaced the simulated OCR/parsing in `server/routes/delivery-routes.ts` with a real call to OpenAI Vision (`gpt-4o-mini`, `response_format: json_object`, `image_url` with `detail: 'high'`). The model is given a strict JSON schema prompt that asks for supplier name, invoice number, delivery date, total amount, and an array of `{ name, barcode, quantity, unitPrice }`. Previously the endpoint returned the same hardcoded list (Coca Cola / Tayto / Brennans Bread / Milk / Cadbury) regardless of what was photographed.
- ✅ Added `fuzzyMatch()` token-overlap matcher: if the parsed barcode equals an existing product's barcode → 0.99 confidence; otherwise tokenise both names (normalise, drop short tokens) and require ≥ 50% overlap. Matched products show in the review UI as "matched to existing stock"; unmatched ones are flagged as new.
- ✅ Failures are explicit — no silent fallback to mock data. Missing API key throws on first call. AI errors return HTTP 502 with the model's reason; zero parsed lines returns 422; the frontend `delivery-scanner.tsx` now surfaces the server's `message` directly in the toast.
- ✅ Body limit: bumped global `express.json` from default 100KB to 1MB; applied `15mb` limit *only* to `/api/delivery/scan-docket` so phone photos go through but other endpoints are not exposed to large-payload abuse.
- ✅ Rate limit: in-memory per-IP token bucket on `/api/delivery/scan-docket` — 12 scans/minute/IP. Returns HTTP 429 "You are scanning too quickly" so a runaway client cannot drain the OpenAI quota.
- ✅ The existing `importDelivery` flow (real DB writes, audit log, purchase order) is unchanged — it already worked. The fix was the eyes-and-brain step that feeds it.
- Modified: `server/routes/delivery-routes.ts`, `server/routes.ts` (route-specific body limit + rate limiter), `server/index.ts` (global body limit), `client/src/components/delivery-scanner.tsx` (error message propagation).
